...
/**
 * DetailPokemon component fetches and displays detailed information about a specific Pokémon.
 *
 * @component
 * @param {Object} props - The component props.
 * @param {string} props.pokeDetailName - The name of the Pokémon to fetch details for.
 * @param {boolean} props.isDetail - A flag to determine if the detail view should be displayed.
 * @returns {JSX.Element} The rendered component.
 *
 * @example
 * <DetailPokemon pokeDetailName="pikachu" isDetail={true} />
 */
export default function DetailPokemon({pokeDetailName, isDetail}) {
    ...
    console.log(pkkData)
    return (
    <div className='fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black-b max-w-[900px] w-full max-h-[400px]
     rounded-4xl max-sm:overflow-y-scroll scrollbar m-2'>
        {isDetail && pkkData.map((pokemon) => {
            const type = pokemon.types[0].type.name
            const type2 = pokemon.types[1] ? pokemon.types[1].type.name : ''
            return (
              <div key={pokemon.id} className='flex flex-col md:flex-row gap-10 ml-10 mr-10  '>
                <div  className={`flex flex-col gap-5  items-center max-w-[250px] w-full  
                    h-[400px] relative`}>
                      <img className='w-66 absolute bottom-52 sm:bottom-72'src={pokemon.sprites.other.home.front_default}></img>
                ...
                <div className='flex  justify-between md:flex-col items-center sm:gap-4'>
                    <div className='w-[1px] h-9 md:h-32 bg-white rotate-[90deg] md:rotate-0 to-100%'></div>
                    ...
                    <div className='w-[1px]  h-9 md:h-32 bg-white rotate-[90deg] md:rotate-0 '></div>
                </div>
                <div className='flex flex-col text-2xl gap-2 mt-10'>
